...
def ProcessRaceScores(grand_prix_name):
...
#getting value of return function call proving elusive
    check_conditions=[]
    check_conditions.append(Condition(validate_grand_prix.IsGrandPrixKnown(grand_prix_name), "\nSorry, I don't recognise the " + grand_prix_name + " GP as being valid."))
    check_conditions.append(Condition(validate_grand_prix.DoResultsExistForGrandPrix(grand_prix_name), "\nSorry, the " + grand_prix_name + " GP has not taken place yet."))
    check_conditions.append(Condition(validate_predictions.DoPredictionsExistForGrandPrix(grand_prix_name),"\nSorry, no predictions for " + grand_prix_name + " are known."))
